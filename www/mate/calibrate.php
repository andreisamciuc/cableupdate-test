...
class Example1 extends Common
{
	...
	function displayHtml()
	{
		?>
			...
			<form name="input" action="calib.php" method="post">
			Line Number: <input type="text" name="cable" value='<?php echo get_config('calibrate');?>' >
			<input type="submit" name="submit" value="Start"><input type="submit" name="submit" value="Stop">
			</form> 
<br /><br /><br />
	...
	{
	        $tableColumns['test_id']     = array('display_text' => 'Test Id',          'perms' => '');
		$tableColumns['cable_number']= array('display_text' => 'Line Description', 'perms' => 'VCTXQ');
		$tableColumns['test_time']  = array('display_text' => 'Time', 'perms' => 'VCTXQ');
		$tableColumns['length'] = array('display_text' => 'Length', 'perms' => 'VCTXQ');
		...
		$this->Editor->setConfig('orderByColumn','test_id');
		$this->Editor->setConfig('ascOrDesc','desc');
		//$this->Editor->setConfig('modifyRowSets',array(&$this,'changeBgColor'));
		$this->Editor->setConfig('numResults','100');
	}

    function changeBgColor($rowSets,$rowInfo,$rowNum)
    {
       if ($rowInfo['result'] == "OK"){
          $rowSets['bgcolor']= '#88ff88';
       }else $rowSets['bgcolor']= '#ff6633';
       //else if
       return $rowSets;
    }
