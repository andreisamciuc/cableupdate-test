...
$(document).ready(function() {
    ...
    $('ul#main_menu li ').hover(function() {
        ...
        $('#' + parent + ' ul:visible ').live({mouseleave: function() {
                $(this).hide();
            }});
    });
    // Fancybox 
    $(".pop_box").fancybox({'showCloseButton': false, 'hideOnOverlayClick': true});
    ...
    $('.data_table3').dataTable({
        ...
        "aoColumns": [
            ...
            /* E-MAIL */  null,
            /* Phone */    null,
            /* Website */ {"bVisible": false},
            /* Company */ null,
            /* Attachaments */ null,
            /* Last Activity */{"bVisible": false},
            /* Tasks */ {"bSortable": false}
    ...
    $("div.toolbar").html('<div class="float:right;"><button class="btn btn-danger" id="deleteall" title="Delete all leads" rel="1"><i class="icon-trash"></i>Flush Leads</button></div>');
    // WYSIWYG Editor
...
$(function() {
    ...
    $("#dateMasked").mask("99/99/9999", {completed: function() {
            alert("completed!");
        }});
    ...
    $("#po").mask("PO: aaa-999-***");
    $("#pct").mask("99%", {completed: function() {
            Processgraph();
        }});
    ...
    $('#alertMessage').live('click', function() {
        $(this).stop(true, true).animate({opacity: 0, right: '-20'}, 500, function() {
            $(this).hide();
        });
    });
...
function fileDialogCallback(callback, type, input) {
    $('<div id="finder_' + callback + '"/>').elfinder({
        url: 'components/elfinder/connectors/php/connector-' + type + '.php', editorCallback: function(url) {
            $('#' + input).val(url);
...
function alertHide() {
    $('#alertMessage').each(function(index) {
        $(this).attr("id", "alertMessage" + index).animate({opacity: 0, right: '30'}, 500, function() {
            $(this).remove();
        });
    });
}
